...
interface PharmacySMSPanelProps {
  isOpen: boolean;
  onClose: () => void;
  pharmacy: {
    id: number;
...
const PharmacySMSPanel: React.FC<PharmacySMSPanelProps> = ({
  isOpen,
  onClose,
  pharmacy,
  darkMode
  ...
  const loadMessages = async () => {
    try {
      setLoading(true);
      const data = await messageService.getDoctorMessages(pharmacy.id);
  ...
  const stopDailyRepeat = async (msg: Message) => {
    if (!msg.daily_repeat) return;
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/messages/${msg.id}/`, {
        ...
        body: JSON.stringify({ daily_repeat: false })
      });
      if (!res.ok) {
      ...
      setMessages(prev => prev.map(m => m.id === msg.id ? { ...m, daily_repeat: updated.daily_repeat ?? false } : m));
      toast({ title: 'Muvaffaqiyat', description: 'Takrorlanish o\'chirildi' });
    } catch (error) {
      console.error('Stop daily repeat error:', error);
      toast({ variant: 'destructive', title: 'Xatolik', description: 'Takrorlanishni o\'chirishda xatolik' });
    } finally {
...
export default PharmacySMSPanel;